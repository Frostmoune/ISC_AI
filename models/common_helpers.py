...
def get_learning_rate(optimizer, global_step, steps_per_epoch):
    with tf.device("/device:CPU:0"):
        ...
        elif lr_decay_mode.startswith("piecewise"):
            boundary = get_dict_default(optimizer, "boundary", "none")
            value = get_dict_default(optimizer, "value", "none")
            if boundary != "none":
                boundary = [int(x) for x in boundary.split(" ")]
